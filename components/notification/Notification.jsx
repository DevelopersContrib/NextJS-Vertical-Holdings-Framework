  ...
  return (
    <AnimatePresence>
      {isVisible && notifications.length > 0 && (
        ...
        >
          <div className="tw-relative tw-flex tw-items-center tw-border tw-border-slate-600 tw-bg-slate-900 tw-rounded-full tw-shadow-[0_8px_32px_-8px_rgb(0_0_0_/_55%)] lg:tw-px-6 lg:tw-py-4 lg:tw-w-[420px] sm:tw-w-[340px] tw-w-full sm:tw-max-w-[95vw] tw-px-3 tw-py-2 tw-gap-[1rem]">
            {/* Badge */}
            ...
            <div className="tw-flex-1 tw-flex tw-flex-col tw-justify-center tw-mr-8 sm:tw-mr-6">
              <span className="lg:tw-text-base md:tw-text-sm sm:tw-text-xs tw-font-medium tw-text-slate-200">
                {` ${notifications[currentIndex].name} bought ${notifications[currentIndex].description} Adao`}
              </span>
              <span className="tw-text-xs tw-text-slate-500 tw-mt-1">
            ...
            <button
              onClick={handleClose}
              className={`tw-absolute tw-right-4 tw-top-1/3 -tw-translate-y-1/2 tw-p-1 tw-rounded-full tw-bg-transparent hover:tw-bg-slate-800 tw-transition-colors tw-text-slate-500 tw-border-none ${
